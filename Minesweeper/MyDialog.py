...
class MyDialog_GameFiles(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        Label(master, text="Open files for the game", justify=LEFT).\
        ...
        self.game_file = StringVar()
        Label(master, text="Game file:").grid(row=1, sticky=W, pady=5, padx=10)
        self.e1 = Entry(master, textvariable=self.game_file)
        self.e1.grid(row=1, column=1, padx=10)
        self.var_game_file = IntVar()
        self.btn_game_file = Button(master, text="...")
        self.btn_game_file.grid(row=1, column=2, sticky=W, pady=5, padx=10)
        self.btn_game_file.bind('<Button-1>', self.GameFileClick)

        self.other_file = StringVar()
        Label(master, text="Other file:").grid(row=2, sticky=W, pady=5, padx=10)
        self.e2 = Entry(master, textvariable=self.other_file)
        self.e2.grid(row=2, column=1, padx=10)
        self.var_other_file = IntVar()
        self.btn_other_file = Button(master, text="...")
        self.btn_other_file.grid(row=2, column=2, sticky=W, pady=5, padx=10)
        self.btn_other_file.bind('<Button-1>', self.OtherFileClick)
    ...
    def apply(self):
        ...
        self.game_file = self.e1.get()
        self.other_file = self.e2.get()
        print('game file: {} \nother file: {} '.format(self.game_file, self.other_file))
        self.result = {'game_file': self.game_file, 'other_file': self.other_file}
    ...
    def validate(self):
        ...
        # import tkMessageBox
        import tkinter.messagebox as messageBox
        if (self.game_file == '' or self.other_file == ''):
            if messageBox.askyesno('Warning', 'no files specified for the game, proceed?', icon="warning"):
                return 1
        else:
            return 0
    ...
    def GameFileClick(self, event):
        print('function: {}'.format(sys._getframe().f_code.co_name))

        self.var_game_file = 1
        self.var_other_file = 0

        self.SelectFile()

    def OtherFileClick(self, event):
        print('function: {}'.format(sys._getframe().f_code.co_name))

        self.var_game_file = 0
        self.var_other_file = 1
    ...
    def SelectFile(self):
        ...
        if self.var_game_file == 1:
            self.game_file.set(filename)
        elif self.var_other_file == 1:
            self.other_file.set(filename)
...
class MyDialog_MazeGeneratorSetup(tkSimpleDialog.Dialog):
